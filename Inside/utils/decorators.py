    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.COMMAND_HAND_LER) == 2:
                superthonreg = "^" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER[1]
            elif len(Config.COMMAND_HAND_LER) == 1:
                superthonreg = "^\\" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER
            args["pattern"] = re.compile(superthonreg + pattern)
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_COMMAND_HAND_LER) == 2:
                superthonreg = "^" + Config.SUDO_COMMAND_HAND_LER
...
def register(**args):
    ...
    def decorator(func):
        if not disable_edited:
            superthon.add_event_handler(func, MessageEdited(**args))
        superthon.add_event_handler(func, NewMessage(**args))
        try:
